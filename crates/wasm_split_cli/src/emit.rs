        ...
        let per_segment = per_segment
            ...
            .map(|(segment_index, segment)| match segment {
                ...
                DataSegmentAnalysis::Ranges {
                    ...
                    // check that range_lookup completely covers the data segment? Otherwise there is non-relocated data,
                    // which indicates an error most likely, or data that is not covered by symbols.
                    // There might be data symbols that are not included anywhere though. Hopefully not.
                    // There are definitely gaps though, introduced by padding for alignment! So for the moment, don't bother.
