...
pub fn get_dependencies(module: &InputModule) -> Result<DepGraph> {
    ...
    for dep_entry in module.iter_data_dependencies() {
        match dep_entry? {
...
impl<'a> InputModule<'a> {
    fn iter_functions_with_relocs(
        ...
        let code_section_offset = self.reloc_info.code_section_offset();
    ...
    fn iter_data_dependencies(&self) -> impl Iterator<Item = Result<DataDependency<'_>>> {
